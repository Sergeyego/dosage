...
bool FormRepPack::addRasx(int row)
{
    ...
    query.bindValue(":prod",modelPart->data(modelPart->index(row,6),Qt::EditRole).toDouble());
    query.bindValue(":break",modelPart->data(modelPart->index(row,7),Qt::EditRole).toDouble());
    query.bindValue(":wast",modelPart->data(modelPart->index(row,8),Qt::EditRole).toDouble());
    query.bindValue(":grab",modelPart->data(modelPart->index(row,9),Qt::EditRole).toDouble());
    query.bindValue(":dry",modelPart->data(modelPart->index(row,10),Qt::EditRole).toDouble());
    bool ok=query.exec();
    if (!ok){
...
{
    QString qu=QString("select p.id, p.n_s, p.dat_part, e.marka, p.diam, ev.nam, kvos.upack, kvos.brk, kvos.loss, kvos.garb, kvos.zames "
                       "from "
                       ...
                       "                 ) as pids "
                       "            where id_part>0 order by id_part ) as parts "
                       "        left join "
    ...
    if (modelPart->execQuery(query)){
        ...
        modelPart->setHeaderData(5,Qt::Horizontal,tr("Вариант"));
        modelPart->setHeaderData(6,Qt::Horizontal,tr("Упаков."));
        modelPart->setHeaderData(7,Qt::Horizontal,tr("В брак"));
        modelPart->setHeaderData(8,Qt::Horizontal,tr("Потери прес."));
        modelPart->setHeaderData(9,Qt::Horizontal,tr("Отх. мусор"));
        modelPart->setHeaderData(10,Qt::Horizontal,tr("Шихта брак"));
...
void FormRepPack::showReportTbl()
{
    if (calc()){
        QSqlQuery query;
        query.prepare("select * from rasxod.prep()");
        if (query.exec()){
            QString title=tr("Расход компонентов на упакованные электроды с %1 по %2").arg(ui->dateEditBeg->date().toString("dd.MM.yy")).arg(ui->dateEditEnd->date().toString("dd.MM.yy"));
            FormReport *w = new FormReport(title);
            w->setAttribute(Qt::WA_DeleteOnClose);
            w->show();
        } else {
            QMessageBox::critical(this,tr("Ошибка"),query.lastError().text(),QMessageBox::Ok);
